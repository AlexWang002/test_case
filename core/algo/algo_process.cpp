        ...
        while (false == to_exit_handle_.load()) {
            ...
            if (sendEnoughData(algo_func_.VIEW_W - 1)) {
                ...
                for (int col_idx = 0; col_idx < algo_func_.VIEW_W; col_idx++)
                {
                    ...
                    uint8_t* denoise_mask_out = algo_func_.denoise_mask_out_frm[col_idx];
                ...
                if (recvEnoughData(frame_buffer)) {
                    ...
                    if(task_id == 0){
                        if (algo_func_.algo_Param.DenoiseOn) {
                            ...
                            memcpy(algo_func_.denoise_mask_out_frm[0], (uint8_t *)&denoise_mask_buffer_h[2 * algo_func_.VIEW_H], (algo_func_.VIEW_W - 2) * algo_func_.VIEW_H * sizeof(int));
                            memcpy(algo_func_.denoise_mask_out_frm[algo_func_.VIEW_W - 2], (uint8_t *)denoise_mask_buffer_h, 2 * algo_func_.VIEW_H * sizeof(int)); // 拷贝最后4列
                        }
                        else {
