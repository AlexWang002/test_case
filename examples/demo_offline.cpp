...
std::vector<FileInfo> getSortedFileInfos(const std::string& kPath) {
    ...
    // 按序号升序排序
    std::sort(infos.begin(), infos.end(), [](const FileInfo& kA, const FileInfo& kB) {
...
LidarSdkErrorCode pointCloudCallback(LidarSensorIndex sensor,
    ...
    last_frame_timestamp = kLidarCloud->frame_timestamp;
    {
        std::cout << "[LidarPointCloudPackets Info :" << kLidarCloud->frame_seq
                  ...
                  << ", return_mode:"
                  << static_cast<uint32_t>(kLidarCloud->return_mode)
                  << ", sync_status:"
                  << static_cast<uint32_t>(kLidarCloud->sync_status)
                  << ", frame_sync:"
...
void printLidarDeviceInfo(const LidarDeviceInfo& kInfo) {
    ...
    uint16_t firmware_version =
        kInfo.firware_version; // 注：原拼写"firware"建议修正为"firmware"
 ...
 ******************************************************************************/
bool readHexDataFromFile(const std::string& kFilePath,
...
void convertToHex(const std::vector<char>& kBuffer, size_t frame_idx) {
    ...
    std::ofstream out_file(lidar_data_path + "/" +
    ...
    if (out_file.is_open()) {
        for (size_t i = 0; i < kBuffer.size(); i++) {
            ...
            out_file << std::hex << std::setw(2) << std::setfill('0')
                     << static_cast<int32_t>(
                            static_cast<std::uint8_t>(kBuffer[i]))
                     << " ";
        }
        out_file << std::endl;
        out_file.close();
        std::cout << "save data to " << lidar_data_path << "/"
...
void getDataFromOsApi(LidarSdkInterface* lidar_interface) {
    ...
    while (!is_exit) {
        ...
        auto duration = std::chrono::duration_cast<std::chrono::microseconds>(
            end_time - start_time);
        auto sleepTime =
            std::chrono::microseconds((int64_t)(kIntervalMs * 1000)) - duration;
...
int main(int argc, char* argv[]) {
    ...
    if (!pathExists(lidar_data_path)) {
        std::cout << "错误: 指定的路径不存在: " << lidar_data_path
                  << std::endl;
        return 1;
    }
