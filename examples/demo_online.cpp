...
LidarSdkErrorCode pointCloudCallback(LidarSensorIndex sensor,
    ...
    static uint64_t max_interval = 101000;
    static uint64_t min_interval = 99000;

    if ((frame_time_interval > (max_interval) ||
                        frame_time_interval < min_interval) || lidar_cloud->frame_seq % 60 == 0) {
...
LidarSdkErrorCode deviceInfoCallback(LidarSensorIndex sensor,
    ...
    device_frame_seq++;
#if 0
    static uint64_t last_recv_device_info_timestamp = 0;
    static bool is_first_recv = true;  // 新增：标记首次接收

    // 新增：用于平滑计算的指数移动平均(EMA)
    static float ema_recv_fps = 0.0f;
    static float ema_frame_fps = 0.0f;
    static const float ALPHA = 0.2f;   // EMA 平滑因子，取值范围 [0,1]

    uint64_t current_recv_device_info_timestamp = getTimeNowPhc();
    uint64_t recv_time_interval = 0;

    if (is_first_recv) {
        is_first_recv = false;
        // 首次接收数据，初始化时间戳，不计算帧率
        last_recv_device_info_timestamp = current_recv_device_info_timestamp;
        std::cout << "First data packet received. Initializing timestamps..." << std::endl;
    } else {
        ...
        if (ema_recv_fps == 0.0f) {
            ...
            ema_recv_fps = ALPHA * instant_recv_fps + (1.0f - ALPHA) * ema_recv_fps;
        }
    }
...
bool initSensorIpcClient() {
    ...
    if (ret != ClientErrorCode::CLIENT_SUCCESS) {
        ...
        auto ret = lidar_client_->getSensorStatus(linkMask, videoMask);
    ...
    while (!is_exit_) {
        ...
        if (ret != ClientErrorCode::CLIENT_SUCCESS) {
            std::cout << "lidar_client_ getFrame failed, error code is: "
                      << static_cast<std::int32_t>(ret) << std::endl;
            uint32_t linkMask, videoMask;
            ret = lidar_client_->getSensorStatus(linkMask, videoMask);
            if (ret != ClientErrorCode::CLIENT_SUCCESS) {
                std::cout
...
#if 0
               static uint64_t last_inject_timestamp = 0;
               static bool is_first_inject = true;  // 新增：标记首次接收
               // 新增：用于平滑计算的指数移动平均(EMA)
