...
LidarSdkErrorCode releaseAdc(LidarSensorIndex sensor, const void* kVoidPtrAdc) {
    ...
    LidarAdcBuffer* ptrAdc = static_cast<LidarAdcBuffer*>(const_cast<void*>(kVoidPtrAdc));
    if (ptrAdc != nullptr) {
        FrameBufObj* frame_obj = static_cast<FrameBufObj*>(const_cast<void*>(ptrAdc->reservedPtr));
        if (frame_obj != nullptr) {
            if (lidar_client) {
                ...
                if (ret == ClientErrorCode::CLIENT_SUCCESS) {
                    ...
                    std::cout << "lidar client release frame failed: " << static_cast<std::int32_t>(ret) << std::endl;
                }
            } else {
...
void saveRawData(const void* kMipiData, uint32_t length, int32_t fileCounter) {
    std::string filename =
        save_path + "pc_bin/lidar_data_" + std::to_string(fileCounter) + "_" + std::to_string(save_count) + ".bin";

    std::ofstream outFile(filename, std::ios::binary);
    if (outFile.is_open()) {
        outFile.write(reinterpret_cast<const char*>(kMipiData), length);
        outFile.close();
        std::cout << "Saved data to: " << filename << ", (size: " << length << " bytes) " << std::endl;
    } else {
...
LidarSdkErrorCode pointCloudCallback(LidarSensorIndex sensor, const void* kBuffer, uint32_t length) {
    ...
    float recv_fps {1000000.0f / recv_time_interval};
    float frame_fps {1000000.0f / frame_time_interval};
    static uint64_t max_interval {102000UL};
    ...
    if (((frame_time_interval > (max_interval)) || (frame_time_interval < min_interval)) ||
        ...
        std::cout << "[LidarPointCloudPackets Info :" << kLidarCloud->frame_seq << "]: "
                  ...
                  << ", lidar_parameter_length:" << kLidarCloud->lidar_parameter_length
                  ...
                  << ", packet_number:" << kLidarCloud->packet_num << ", last_fram_seq:" << last_fram_seq
                  ...
                  << ", counter:" << static_cast<uint32_t>(kLidarCloud->counter) << std::endl;
...
void printLidarDeviceInfo(const LidarDeviceInfo& kInfo) {
    ...
    // 打印故障码信息
    std::cout << "SDK Total Fault Number: " << static_cast<int32_t>(kInfo.sdk_total_fault_number) << std::endl;
    ...
    // 打印供应商内部故障信息1
    std::cout << std::endl << "Supplier Internal Fault:" << std::endl;
    std::cout << "  Supplier Internal Fault ID1: 0x" << std::hex
              << static_cast<int32_t>(kInfo.supplier_internal_fault_id) << std::dec << std::endl;
    std::cout << "  Supplier Internal Fault Indicate1: ";
    ...
    std::cout << std::endl;
    std::cout << "  Supplier Internal Fault Indicate2: ";
    for (size_t i = 2; i < 6; ++i) {
        std::cout << std::hex << std::setw(2) << std::setfill('0') << static_cast<int32_t>(kInfo.reserved[i])
                  << std::dec << " ";
    }
    std::cout << std::endl;
    std::cout << "  Supplier Total Internal Fault Number: " << static_cast<int32_t>(kInfo.reserved[6]) << std::endl;
    std::cout << "  Supplier Total Internal Fault Position: ";
    ...
    std::cout << "  Time Sync Mode: 0x" << std::hex << static_cast<int32_t>(kInfo.time_sync_mode) << std::dec
              << std::endl;
    std::cout << "  Time Sync Status: " << (kInfo.time_sync_status == 0x01 ? "Succeeded" : "Failed") << " (0x"
              << std::hex << static_cast<int32_t>(kInfo.time_sync_status) << ")" << std::dec << std::endl;
    std::cout << "  Timing Offset: " << kInfo.time_offset << " μs" << std::endl;

    // 打印产品序列号（十六进制字符串）
    std::cout << "LidarProduct_SN_String: ";
...
LidarSdkErrorCode writeSensorRegUint16(LidarSensorIndex sensor, uint16_t address, uint16_t data) {
    if (!lidar_client) {
        std::cout << "writeSensorRegUint16 error: lidar_client is nullptr" << std::endl;
        return LidarSdkErrorCode::LIDAR_SDK_FAILD;
    }
    std::cout << "writeSensorRegUint16: address: " << address << ", data: " << data << std::endl;
    ClientErrorCode clientRet = lidar_client->writeSensorRegUint16(SensorType::MIDDLE_LIDAR, address, data);
    std::cout << "writeSensorRegUint16: clientRet: " << static_cast<int32_t>(clientRet) << std::endl;
    return clientRet == ClientErrorCode::CLIENT_SUCCESS ? LidarSdkErrorCode::LIDAR_SDK_SUCCESS
...
LidarSdkErrorCode readSensorRegUint16(LidarSensorIndex sensor, uint16_t address, uint16_t* data) {
    if ((!lidar_client) || (!data)) {
        std::cout << "readSensorRegUint16 error: lidar_client is nullptr" << std::endl;
        return LidarSdkErrorCode::LIDAR_SDK_FAILD;
    }
    std::cout << "readSensorRegUint16: address: " << address << ", data: " << *data << std::endl;
    ClientErrorCode clientRet = lidar_client->readSensorRegUint16(SensorType::MIDDLE_LIDAR, address, *data);
    std::cout << "readSensorRegUint16: clientRet: " << static_cast<int32_t>(clientRet) << std::endl;
    return clientRet == ClientErrorCode::CLIENT_SUCCESS ? LidarSdkErrorCode::LIDAR_SDK_SUCCESS
                                                        : LidarSdkErrorCode::LIDAR_SDK_FAILD;
}
...
LidarSdkErrorCode writeSensorRegUint8(LidarSensorIndex sensor, uint16_t address, uint8_t data) {
    if (!lidar_client) {
        std::cout << "[writeSensorRegUint8]  lidar_client is nullptr " << std::endl;
        return LidarSdkErrorCode::LIDAR_SDK_FAILD;
    }
    std::cout << "writeSensorRegUint8: address: " << address << ", data: " << data << std::endl;
    ClientErrorCode clientRet = lidar_client->writeSensorRegUint8(SensorType::MIDDLE_LIDAR, address, data);
    std::cout << "writeSensorRegUint8: clientRet: " << static_cast<int32_t>(clientRet) << std::endl;
    return clientRet == ClientErrorCode::CLIENT_SUCCESS ? LidarSdkErrorCode::LIDAR_SDK_SUCCESS
                                                        : LidarSdkErrorCode::LIDAR_SDK_FAILD;
}
...
LidarSdkErrorCode readSensorRegUint8(LidarSensorIndex sensor, uint16_t address, uint8_t* data) {
    if ((!lidar_client) || (!data)) {
        std::cout << "readSensorRegUint8 error: lidar_client is nullptr " << std::endl;
        return LidarSdkErrorCode::LIDAR_SDK_FAILD;
    }
    std::cout << "readSensorRegUint8: address: " << address << ", data: " << *data << std::endl;
    ClientErrorCode clientRet = lidar_client->readSensorRegUint8(SensorType::MIDDLE_LIDAR, address, *data);
    std::cout << "readSensorRegUint8: clientRet: " << static_cast<int32_t>(clientRet) << std::endl;
    return clientRet == ClientErrorCode::CLIENT_SUCCESS ? LidarSdkErrorCode::LIDAR_SDK_SUCCESS
                                                        : LidarSdkErrorCode::LIDAR_SDK_FAILD;
}
...
LidarSdkErrorCode readSensorI2C(LidarSensorIndex sensor, uint16_t address, uint8_t* data, uint16_t* length) {
    if (((!data) || (!length)) || (!lidar_client)) {
        std::cout << "readSensorI2C error: lidar_client is nullptr " << std::endl;
        return LidarSdkErrorCode::LIDAR_SDK_FAILD; // 数据缓冲区和长度指针不可为空
    }

    std::cout << "readSensorI2C: address: " << address << ", expected length: " << *length << std::endl;
    // 模拟数据填充（仅用于示例）
    for (int32_t i = 0; i < *length; i++) {
        data[i] = i;
    }

    ClientErrorCode clientRet = lidar_client->readSensorI2C(SensorType::MIDDLE_LIDAR, address, data, *length);
    std::cout << "readSensorI2C: clientRet: " << static_cast<int32_t>(clientRet) << std::endl;
    return clientRet == ClientErrorCode::CLIENT_SUCCESS ? LidarSdkErrorCode::LIDAR_SDK_SUCCESS
                                                        : LidarSdkErrorCode::LIDAR_SDK_FAILD;
}
...
LidarSdkErrorCode writeSensorI2C(LidarSensorIndex sensor, uint16_t address, const uint8_t* kData, uint16_t length) {
    ...
    ClientErrorCode clientRet = lidar_client->writeSensorI2C(SensorType::MIDDLE_LIDAR, address, kData, length);
    std::cout << "writeSensorI2C: clientRet: " << static_cast<int32_t>(clientRet) << std::endl;
    return clientRet == ClientErrorCode::CLIENT_SUCCESS ? LidarSdkErrorCode::LIDAR_SDK_SUCCESS
                                                        : LidarSdkErrorCode::LIDAR_SDK_FAILD;
}
...
bool initSensorIpcClient() {
    lidar_client =
        std::make_shared<dimw::sensor_access::SensorIpcClient>(dimw::sensor_access::AppType::SEN_IRC_LIDAR_CONSUMER);
    uint32_t mask = 0b100000000000000;
    ClientErrorCode ret = lidar_client->init(mask);

    if (ret != ClientErrorCode::CLIENT_SUCCESS) {
        std::cout << "lidarClient init failed, error code is: " << static_cast<std::int32_t>(ret);
        // uint32_t linkMask, videoMask;
        // auto ret = lidar_client->getSensorStatus(linkMask, videoMask);
        // std::cout << "getSensorStatus: " << std::to_string(static_cast<std::int32_t>(ret))
        //           << ", linkMask: " << (int)linkMask << ", videoMask: " << (int)videoMask;
        return false;
    }

    ret = lidar_client->start();
    if (ret != ClientErrorCode::CLIENT_SUCCESS) {
        std::cout << "lidarClient start failed, error code is: " << std::to_string(static_cast<std::int32_t>(ret));
        return false;
    }
    return true;
}
...
void getDataFromOsApi(LidarSdkInterface* lidar_interface) {
    ...
    while (!is_exit) {
        ...
        auto ret = lidar_client->getFrame(SensorType::MIDDLE_LIDAR, *frame, false);

        if (ret != ClientErrorCode::CLIENT_SUCCESS) {
            std::cout << "lidar_client getFrame failed, error code is: " << static_cast<std::int32_t>(ret) << std::endl;
            // uint32_t linkMask {0};
            // uint32_t videoMask {0};
            // ret = lidar_client->getSensorStatus(linkMask, videoMask);

            // if (ret != ClientErrorCode::CLIENT_SUCCESS) {
                // std::cout << "lidarClient get sensor status failed, error code is: " << static_cast<std::int32_t>(ret);
            // } else {
        ...
        buf->teof = frame->teof;
        buf->tsof = frame->tsof;
        buf->bufObj = static_cast<void*>(frame->bufObj);
        buf->reservedPtr = frame;

#if 0
        std::cout << "data_valid: " << json_reader::test_param.data_valid << std::endl;
        std::cout << "enable_mipi_crc: " << json_reader::test_param.enable_mipi_crc << std::endl;

        // CRC test
        if (json_reader::test_param.data_valid && json_reader::test_param.enable_mipi_crc) {
        ...
        if (frame->data == nullptr) {
            std::cout << "frame->data is nullptr" << std::endl;
        }
        ...
        LidarSdkErrorCode inject_ret = lidar_interface->injectAdc(LidarSensorIndex::MIDDLE_LIDAR, buf);
...
int32_t main(int32_t argc, char* argv[]) {
    ...
    for (int32_t i = 1; i < argc; ++i) {
        ...
        if ((arg == "-h") || (arg == "--help")) {
            ...
            if (i + 1 < argc) {
                ...
                if (test_num <= 0) {
                    std::cout << "错误：-n/--num 选项需要指定大于0的次数参数" << std::endl;
                    return 1;
                }
            } else {
    ...
    if (!save_pcd && json_reader::test_param.data_valid) {
        save_pcd = json_reader::test_param.enable_save_pcd;
        save_path = json_reader::test_param.save_path;
    ...
    if (test_stop) {
        ...
        if (test_poweroff) {
            ...
            usleep(700e3);
            lidar_interface->setCalibrationEnable(true);
            usleep(700e3);
            (void)readSensorI2C(LidarSensorIndex::MIDDLE_LIDAR, address, data, &length);
            std::cout << "main, address: " << std::hex << address << ", data: " << (uint32_t)data[0] << ", length: " << std::dec << length << std::endl;
            usleep(700e3);
            lidar_interface->setCalibrationEnable(false);
            usleep(700e3);
            (void)readSensorI2C(LidarSensorIndex::MIDDLE_LIDAR, address, data, &length);
            std::cout << "main, address: " << std::hex << address << ", data: " << (uint32_t)data[0] << ", length: " << std::dec << length << std::endl;
            usleep(700e3);
            ...
            usleep(700e3);
            std::cout << "main, address: " << std::hex << address << ", data: " << (uint32_t)data[0] << ", length: " << std::dec << length << std::endl;
            LidarSdkErrorCode result = lidar_interface->powerOff();
            std::cout << "powerOff result: " << result << std::endl;
            usleep(700e3);
            (void)readSensorI2C(LidarSensorIndex::MIDDLE_LIDAR, address, data, &length);
            usleep(700e3);
            std::cout << "main, address: " << std::hex << address << ", data: " << (uint32_t)data[0] << ", length: " << std::dec << length << std::endl;
